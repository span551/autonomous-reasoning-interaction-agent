...
if st.session_state.last_result:
    ...
    with col_a:
        ...
        # ── Step labels
        stt_label    = f"TRANSCRIPTION ({r['stt_time']}s)"    if r.get("stt_time")    else "TRANSCRIPTION"
        intent_label = f"DETECTED INTENT ({r['intent_time']}s)" if r.get("intent_time") else "DETECTED INTENT"
        exec_label   = f"ACTION EXECUTED ({r['exec_time']}s)"  if r.get("exec_time")   else "ACTION EXECUTED"

        # Card wrapper open
        st.markdown("<div class='card card-accent'>", unsafe_allow_html=True)
        st.markdown("<div style='font-size:11px;color:#6b6b8a;text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px;'>Pipeline Trace</div>", unsafe_allow_html=True)

        # Step 01 — Transcription
        with st.container():
            st.markdown(
                f"<div style='font-size:11px;color:#6b6b8a;text-transform:uppercase;"
                f"letter-spacing:.08em;margin-top:12px;'>{stt_label}</div>",
                unsafe_allow_html=True
            )
            ...
            with c2:
                st.markdown(f'*"{r.get("transcription", "")}"*')
        ...
        with st.container():
            st.markdown(
                f"<div style='font-size:11px;color:#6b6b8a;text-transform:uppercase;"
                f"letter-spacing:.08em;'>{intent_label}</div>",
                unsafe_allow_html=True
            )
            c1, c2 = st.columns([1, 10])
            with c1:
            ...
            with c2:
                badges = f"<span class='tag tag-intent'>{primary}</span>"
                for ci in compound:
                    badges += f"&nbsp;<span class='tag tag-warn'>{ci}</span>"
        ...
        with st.container():
            st.markdown(
                f"<div style='font-size:11px;color:#6b6b8a;text-transform:uppercase;"
                f"letter-spacing:.08em;'>{exec_label}</div>",
                unsafe_allow_html=True
            )
            c1, c2 = st.columns([1, 10])
            with c1:
            ...
            with c2:
                st.markdown(r.get("action_taken", primary))
        ...
        with st.container():
            st.markdown(
                ...
                "letter-spacing:.08em;'>STATUS</div>",
                unsafe_allow_html=True
            )
            c1, c2 = st.columns([1, 10])
            with c1:
            ...
            with c2:
                tag_cls = "tag-success" if status == "success" else "tag-warn"
                st.markdown(
                    f"<span class='tag {tag_cls}'>{status.upper()}</span>"
                    f"<span style='font-size:12px;color:#9090b0;margin-left:8px;'>"
                    f"{r.get('message', '')}</span>",
                    unsafe_allow_html=True
                )
    ...
    with col_b:
        ...
        border = "#34d399" if status == "success" else "#f87171"
        st.markdown(
        ...
        for f in files:
            st.markdown(f"📄 `output/{f}`")

        if output:
            st.code(output[:3000] + ("…" if len(output) > 3000 else ""), language="text")
        else:
